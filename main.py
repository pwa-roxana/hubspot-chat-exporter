...
LOGIN_HTML = """
<!DOCTYPE html><html><head><title>PWA Chat Exporter</title>
...
.logo{font-size:12px;font-weight:700;color:#f97316;letter-spacing:.1em;text-transform:uppercase;margin-bottom:.5rem}
...
MAIN_HTML = """
<!DOCTYPE html><html><head><title>PWA Live Chat Exporter</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f4f6f9;min-height:100vh;display:flex;align-items:center;justify-content:center}
...
.logo{font-size:12px;font-weight:700;color:#f97316;letter-spacing:.1em;text-transform:uppercase;margin-bottom:.5rem}
h1{font-size:22px;font-weight:600;color:#1a1a2e;margin-bottom:.25rem}
p.sub{font-size:14px;color:#6b7280;margin-bottom:1.75rem}
label{display:block;font-size:13px;font-weight:500;color:#374151;margin-bottom:5px}
input{width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;color:#111827;margin-bottom:1rem;outline:none}
input:focus{border-color:#1F3864}
.row{display:grid;grid-template-columns:1fr 1fr;gap:12px}
button.export-btn{width:100%;padding:12px;background:#1F3864;color:white;border:none;border-radius:8px;font-size:15px;font-weight:600;cursor:pointer;margin-top:.25rem}
...
.progress-bar{height:100%;background:#1F3864;width:0%;transition:width .4s;border-radius:4px}
...
<div class="card">
  ...
  <div class="row">
    <div><label>From Date (optional)</label><input type="date" id="from_date"/></div>
    <div><label>To Date (optional)</label><input type="date" id="to_date"/></div>
  </div>
...
async function runExport(){
  ...
  setBar(10);showStatus('Connecting to HubSpot...','info');
  try{
    setBar(30);showStatus('Fetching conversations and messages... this may take a minute.','info');
    const resp=await fetch('/export',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({from_date:document.getElementById('from_date').value||null,to_date:document.getElementById('to_date').value||null})});
    setBar(90);
    if(!resp.ok){const err=await resp.json();throw new Error(err.error||'Export failed');}
    const blob=await resp.blob();
    const url=URL.createObjectURL(blob);
    const date=new Date().toISOString().slice(0,10);
    setBar(100);showStatus('Export complete!','success');
    document.getElementById('dl-area').innerHTML=`<a class="dl" href="${url}" download="hubspot_live_chats_${date}.xlsx">Download Excel File</a>`;
  }catch(e){showStatus('Error: '+e.message,'error');document.getElementById('progress').style.display='none';}
...
def api_data():
    ...
    if not HUBSPOT_TOKEN:
        return jsonify({'error': 'HUBSPOT_TOKEN not set'}), 500
        ...
        for conv in conversations:
            ...
            for msg in messages:
                ...
                result.append({
                    ...
                    'sender': sender,
                    'message_time': msg.get('createdAt',''),
                    'message': msg_text
                })
...
def export():
    ...
    col_headers = ['Thread ID','Date','Status','Visitor Name','Visitor Email','Visitor Phone','Assigned To','Sender','Message Time','Message']
    ...
    for i,conv in enumerate(conversations):
        ...
        if not messages:
            row_data = [tid,created,status,contact_name,contact_email,contact_phone,assigned,'','','']
            for col,val in enumerate(row_data,1):
                cell=ws.cell(row=row_num,column=col,value=val);cell.fill=fill;cell.alignment=Alignment(wrap_text=True,vertical='top')
            row_num+=1
        else:
            for msg in messages:
                sender_type=msg.get('senderType') or msg.get('type') or ''
                ...
                row_data=[tid,created,status,contact_name,contact_email,contact_phone,assigned,sender,msg_time,msg_text]
                for col,val in enumerate(row_data,1):
                    cell=ws.cell(row=row_num,column=col,value=val);cell.fill=fill;cell.alignment=Alignment(wrap_text=True,vertical='top')
                row_num+=1
    ws.freeze_panes='A2'
